...
namespace ThrottleControlledAvionics
{
    public class AttitudePanel : ControlPanel<AttitudeUI>
    {
        ...
        protected override void init_controller()
        {
            ...
            Controller.CuesPanel.Maneuver.onValueChanged.AddListener(state => onCueChange(Attitude.ManeuverNode, state));
            ...
            Controller.CuesPanel.Tp_rVm.onValueChanged.AddListener(state => onCueChange(Attitude.TargetCorrected, state));
            base.init_controller();
        }

        void onCueChange(Attitude cue, bool state)
        {
            if(state)
                CFG.AT.XOnIfNot(cue);
            else
        ...
        void disableCurrentCue() => CFG.AT.XOff();
        ...
        protected override void OnLateUpdate()
        {
            ...
            if(CFG.AT)
            {
                ...
                if(VSL.AutopilotDisabled) 
                    ...
                    err = string.Format("Err: {0:F1}°", VSL.Controls.AttitudeError);
                Controller.CurrentCue.AttitudeError.text = err;
                Controller.CurrentCue.AttitudeError.color = VSL.Controls.Aligned ? Colors.Enabled : Colors.Neutral;

        ...
        class PrettyNode : TCAPartGraphNode
        {
            ...
            public bool Draw(VesselConfig CFG)
            {
                ...
                if(CFG.EnabledTCAParts.Contains(part.Name))
                {
                    ...
                    else if(GUILayout.Button(button,
                    ...
                    foreach(var tier in graph.tiers)
                    {
                        ...
                        foreach(var node in tier.Value)
                        {
                            ...
                            var start = new Vector2(
                                WindowPos.x + sr.x + sr.width,
                                WindowPos.y + sr.y + sr.height / 2);
                            foreach(var next in node.outputs)
                            {
                                if(next.tier != next_tier)
                                    continue;
                                var er = ((PrettyNode)next).button_rect;
                                var end = new Vector2(
                                    WindowPos.x + er.x,
                                    WindowPos.y + er.y + sr.height / 2);
                                Drawing.DrawLine(
                                    start,
                                    end,
                                    next.part.Active ? Colors.Good : inactive,
                                    1,
                                    true);
                            }
                        }

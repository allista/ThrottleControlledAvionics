...
//				Log("Activated: {}\nAll engines: {}\nProfile: {}", cluster, VSL.Engines.All, CFG.ActiveProfile);//debug
        ...
        public float TTB_Precise(float dV)
        {
            ...
            var throttle = ThrottleControl.NextThrottle(dV, 1, mass, MaxThrustM, DecelerationTime);
            while(dV > 0.1)
            {
                throttle = ThrottleControl.NextThrottle(dV, throttle, mass, MaxThrustM, DecelerationTime);
                dV /= 2;

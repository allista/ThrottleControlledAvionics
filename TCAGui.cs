...
namespace ThrottleControlledAvionics
{
	public class TCAGui : MonoBehaviourWindow
	{
		...
		public const int controlsWidth = 400, controlsHeight = 270;
		...
		internal override void DrawWindow(int id)
		{
			...
			GUILayout.BeginHorizontal(); //enclosing horizontal for adv.
			...
			GUILayout.EndHorizontal();//enclosing horizontal for adv.
			GUI.DragWindow();
			ResetWindowSize();
		...
		private void killLines() {
			if(engLine != null)		Destroy (engLine.gameObject);
			if(dirLine != null)		Destroy (dirLine.gameObject);
			if(upLine != null)		Destroy (upLine.gameObject);
			if(gravLine != null)	Destroy (gravLine.gameObject);
			if(vecLine != null)		Destroy (vecLine.gameObject);
			if(engLine != null)		Destroy (engLine.gameObject);
			TCA.KillLines ();
		}
		#endregion

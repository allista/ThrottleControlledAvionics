...
Many of the TCA functions are controlled through *switches*. These are buttons that are **green** when the function is enabled, **yellow** when it is disabled, and gray when it is unavailable.
...
When TCA is bought in R&D, all qualified parts in flight are upgraded using the OTA update.

However, TCA interface and functionality only becomes available if a vessel has some engines and/or RCS thrusters.

...
class ATC(object):
    ...
    def updateAV(self):
        ...
        self.AV += self.AA * dt + (np.random.rand()-0.5)*1e-4
...
if __name__ == '__main__':
    ...
    def tune_steering_mixed(atc, iErrf, imaxAA, AM):
        if atc.InstantRatio > 0.3:
            ...
            atc.atPID.I = 0
            atc.atPID.D = 0
            noise_scale = clamp((50*(abs(atc.AV)+abs(atc.error/np.pi)))**0.5, 0.001, 1)
            atc.avPID.P = ((MixedConfig.avP_A / (atc.InstantRatio ** MixedConfig.avP_D + MixedConfig.avP_B) +
                          ...
                          * noise_scale #clamp(abs(300*(abs(atc.avPID.perror)+abs(atc.error/np.pi)))**0.5, 0.1, 1)
            atc.avPID.D = ((MixedConfig.avD_A / (atc.InstantRatio ** MixedConfig.avD_D + MixedConfig.avD_B) +
                           MixedConfig.avD_C) / atc.MaxAA) \
                          * noise_scale #clampH(abs(40*(abs(atc.avPID.perror)+abs(atc.error/np.pi)))**6, 1)
    ...
    simAngle(wheesly, AAs, 0.7, wheels_ratio, 0, angles)

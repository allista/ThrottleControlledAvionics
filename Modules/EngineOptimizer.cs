...
namespace ThrottleControlledAvionics
{
    public abstract class TorqueOptimizer : TCAModule
    {
        ...
        public abstract class Config<T> : ComponentConfig<T> where T : ComponentConfig, new()
        {
            ...
            /// </summary>
            [Persistent]
            public float OptimizationPrecision = 0.01f;
            ...
            /// </summary>
            [Persistent]
            public float OptimizationAngleCutoff = 45f;

            /// <summary>
            /// linear weight of angle error in the total error = torque_error+angle_error
            /// </summary>
            [Persistent]
            public float AngleErrorWeight = 1f;
            ...
            /// </summary>
            [Persistent]
            public float OptimizationTorqueCutoff = 1f;

            /// <summary>
            /// torque-ratio curve
            /// </summary>
            [Persistent]
            public float TorqueRatioFactor = 0.1f;

            /// <summary>
            /// torque ratio threshold for switching an engine to unbalanced mode
            /// </summary>
            [Persistent]
            public float UnBalancedThreshold = 0.0001f; //<1 deg
    ...
    public class EngineOptimizer : TorqueOptimizer
    {
        ...
        public class Config : Config<Config>
        {
            ...
            /// </summary>
            [Persistent]
            public PI_Controller EnginesPI = new PI_Controller(0.4f, 0.2f);

            /// <summary>
            /// float curve for P value of Engines PI controller = F(torque/MoI)
            /// </summary>
            [Persistent]
            public FloatCurve EnginesCurve = new FloatCurve();
        ...
        )
        {
            ...
            {
                var e = engines[i];
                e.limit_tmp = -Vector3.Dot(e.getCurrentTorque(useDefTorque), target_n)
            ...
            if(compensation_m < eps && maneuver_m.Equals(0))
                return false;
        ...
        public bool OptimizeLimitsForTorque(
            IList<EngineWrapper> engines,
            Vector3 needed_torque,
            bool useDefTorque,
            out float max_limit
        ...
        {
            var ret = OptimizeLimitsForTorque(engines,
                needed_torque,
                VSL.Torque.Imbalance.Torque,
                VSL.Physics.MoI,
                useDefTorque,
                out max_limit,
                out var torque_error,
                out var angle_error);
        ...
        public static void LimitInDirection(IList<EngineWrapper> engines, Vector3 dir)
        {
            ...
            for(var i = 0; i < num_engines; i++)
            {
                ...
                e.limit = e.preset_limit = e.limit * (1 - Mathf.Abs(e.limit_tmp));
            }
        }
        ...
        private void tune_steering_params()
        {
            ...
            if(CFG.AT)
                CFG.SteeringModifier = Vector3.one;
            else
            {

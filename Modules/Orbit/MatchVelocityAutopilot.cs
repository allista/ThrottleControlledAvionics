...
namespace ThrottleControlledAvionics
{
	...
	public class MatchVelocityAutopilot : TCAModule
	{
        ...
        public static float BrakeDistancePrecise(float V0, VesselWrapper VSL, out float ttb)
        {
            ...
            var throttle = ThrottleControl.NextThrottle(V0, 1, mass, VSL.Engines.MaxThrustM, VSL.Engines.DecelerationTime);
            float next_mass;
            float ttb_cur;
            while(V0 > 0.1)
            {
                throttle = ThrottleControl.NextThrottle(V0, throttle, mass, VSL.Engines.MaxThrustM, VSL.Engines.DecelerationTime);

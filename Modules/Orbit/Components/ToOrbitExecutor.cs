...
namespace ThrottleControlledAvionics
{
    public class ToOrbitExecutor : OrbitalComponent
    {
        ...
        public void StartGravityTurn()
        {
            ...
            CFG.HF.Off();
            last_UT = Planetarium.GetUniversalTime();
        }
        ...
        public bool GravityTurn(double ApA_offset, double gturn_curve, double dist2vel, double Dtol)
        {
            ...
            if(!ErrorThreshold)
            {
                ...
                if(arc > Dtol && (!ApoapsisReached || !CorrectOnlyAltitude))
                {
                    ...
                    if(Body.atmosphere) 
                        hvel *= Utils.Clamp(1-VSL.vessel.atmDensity/GLB.ORB.AtmDensityOffset, 0, 1);
                    vel += hv*hvel;
                }
                vel *= VSL.Physics.StG/Utils.G0*dist2vel;
                var srf_vel = VSL.vessel.srf_vel_direction.xzy;
                var angle2prg = Utils.Angle2(vel, srf_vel);
                if(angle2prg > GLB.ATCB.MaxAttitudeError)
                    vel = QuaternionD.AngleAxis(GLB.ATCB.MaxAttitudeError, Vector3d.Cross(srf_vel, vel)) 
                                     * srf_vel * vel.magnitude;
                if(!vel.IsZero())
                {
